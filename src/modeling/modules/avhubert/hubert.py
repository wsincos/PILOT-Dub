...
class AVHubertConfig(FairseqDataclass):
    ...
    required_seq_len_multiple: int = field(
        default=1, 
        metadata={"help": "pad the input sequence length so it is divisible by this value"}
    )
    # add begin
    layer_type: str = field(
        default="transformer", 
        metadata={"help": "layer type, e.g. transformer, conformer"}
    )

    # 2. Dropout 相关参数 (TransformerEncoder 强制检查)
    dropout: float = field(
        default=0.1, 
    ...
    untie_weights_roberta: bool = field(
        default=False,
        metadata={"help": "Untie weights for RoBERTa"}
    )
    ...
    tie_adaptive_weights: bool = field(
        default=False,
        metadata={"help": "if set, ties the weights of adaptive softmax and output layer"}
    )
    ...
    decoder_output_dim: int = field(
        default=768, 
        metadata={"help": "decoder output dimension"}
    )
