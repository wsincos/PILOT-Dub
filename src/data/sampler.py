...
class DistributedDynamicBatchSampler(Sampler):
    ...
    def __iter__(self):
        start_step = self._cur_step
        for i, batch in enumerate(self._replica_batches):
            # Update before yielding so checkpointing after batch end captures progress.
            self._cur_step = start_step + i + 1
            yield batch
    ...
    def load_state_dict(self, state_dict):
        ...
        cur_step = int(state_dict.get("cur_step", 0))
        self.continue_flag = True
        self._cur_step = cur_step
