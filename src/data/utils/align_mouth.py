...
def load_args(default_config=None):
    ...
    parser.add_argument('--ffmpeg', default="ffmpeg", type=str, help='ffmpeg path')
...
def extract_lip(args):
    lines = open(args.filename_path).readlines()
    fids = sorted([ln.strip() for ln in lines])
    ...
    for filename_idx, filename in enumerate(tqdm(fids)):
        ...
        dst_pathname = os.path.join(args.save_direc, id+"_" +utt+ '.mp4')

        if os.path.isfile(dst_pathname): continue

        if not os.path.isfile(landmarks_pathname) or not os.path.isfile(video_pathname):
            # print(landmarks_pathname)
            total_cnt+=1
            continue
...
if __name__ == '__main__':
    args = load_args()
    mean_face_path = os.path.join(args.face_preprocess_dir, "20words_mean_face.npy")
    # -- mean face utils
    STD_SIZE = (256, 256)
